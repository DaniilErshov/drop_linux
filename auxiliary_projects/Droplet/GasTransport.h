...
const double logGasConstant = log(GasConstant);

//! Universal gas constant in cal/mol/K
const double GasConst_cal_mol_K = GasConstant / 4184.0;

//! Stefan-Boltzmann constant @f$ \sigma @f$ [W/m2/K4]
const double StefanBoltz = 2.0 * pow(Pi, 5) * pow(Boltzmann, 4) / (15.0 * pow(Planck, 3) * lightSpeed * lightSpeed); // 5.670374419e-8
...
public:
    ...
    /*!
        ...
        */
    void getBinaryDiffCoeffs(const size_t ld, double* const d) ;
    ...
    //! @see fitProperties()
    void getViscosityPolynomial(size_t i, double* coeffs) const ;

    //! Return the temperature fits of the heat conductivity of species i
    //! @see fitProperties()
    void getConductivityPolynomial(size_t i, double* coeffs) const ;

    //! Return the polynomial fits to the binary diffusivity of species pair (i, j)
    //! @see fitDiffCoeffs()
    void getBinDiffusivityPolynomial(size_t i, size_t j, double* coeffs) const ;
    ...
    //! @see fitProperties()
    void setViscosityPolynomial(size_t i, double* coeffs) ;

    //! Modify the temperature fits of the heat conductivity of species i
    //! @see fitProperties()
    void setConductivityPolynomial(size_t i, double* coeffs) ;

    //! Modify the polynomial fits to the binary diffusivity of species pair (i, j)
    //! @see fitDiffCoeffs()
    void setBinDiffusivityPolynomial(size_t i, size_t j, double* coeffs) ;
...
protected:
    ...
    /*!
        ...
        *  @endcode
        */
    vector<vector<double>> m_wratjk;

    //! Holds square roots of molecular weight ratios
    /*!
        *  `m_wratjk1(j,k)  = sqrt(1.0 + mw[k]/mw[j])        j < k`
        */
    vector<vector<double>> m_wratkj1;
    ...
    /*!
        ...
        */
    vector<vector<double>> m_reducedMass;
    ...
    /*!
        ...
        * Length nsp * nsp. This is a symmetric matrix.
        */
    vector<vector<double>> m_epsilon;
