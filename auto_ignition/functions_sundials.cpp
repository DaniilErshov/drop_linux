...
int integrate_All_IDA_M(int N_x) {
    ...
    for (int i = 1; i < N_x - 1; i++) {
        ...
        if (i > preinter + 1) {
            for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
                consval[i_temp] = 1.0 - need_second_start;   /*constraint*/
                yval[i_temp] = Cell_Properties_vector[i].Y[k_spec];
                atval[i_temp] = pow(10, -7);
                id_val[i_temp] = 1.;
                i_temp++;
    ...
    retval = IDAInit(mem, func_All_IDA_M, t_curr, yy, yp);
    ...
    double W, w_dot;
    ofstream params;
    params.open(".//auto_ignition//params//params.dat");
    ...
    while (t_curr <= t_end_all) {
        ...
        if (iout % n_out == 0) {
            Write_to_file(".//auto_ignition//ida_result//" + to_string(tout * pow(10, 0)) + "_yval", "yval", Cell_Properties_vector, Cell_Properties_inter);
            Write_drhodt(".//auto_ignition//ida_result//" + to_string(tout * pow(10, 0)) + "drhodt");
            Write_to_file(".//auto_ignition//ida_result//" + to_string(tout * pow(10, 0)) + "_ypval", "ypval", Cell_prouds_vector, Cell_prouds_inter);
        }
        if (iout % n_out == 0) {
...
#pragma omp for schedule(dynamic)
        for (int i = 1; i < Nx - 1; i++) {
            ...
            if (i > preinter)
            {
                //if (abs(Cell_Properties_vector[i].Y[komponents[Fuel]]) > pow(10, -12)) {
                //    Get_molar_cons(Xi, Cell_Properties_vector[i].Y.data(), Cell_Properties_vector[i].T);
                //    chem_vel(Sn, Hn, forward_arr, reverse_arr, equilib_arr,
                //        Cell_Properties_vector[i].T, Xi, ydot, i);
                //}
