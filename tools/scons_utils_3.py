...
INIT_CC_FLAGS = ' -O2 -Wall -Wextra -Wno-unused-parameter -Wno-sign-compare'
...
class PlatformSpec(object):
    ...
    def FixLibsFiles(self, File):
        def _fixer(s):
            ...
            return s
        self.system_libs = list(map(_fixer, self.system_libs))
        self.third_party_libs = list(map(_fixer, self.third_party_libs))
...
class PlatformSpecCluster(PlatformSpec):
    ...
    def __init__(self, env, options):
        ...
        self.ccflags += ' -qmkl=sequential'
        self.fortranflags += ' -qmkl=sequential'
        self.linkflags += ' -qmkl=sequential'
        self.num_jobs = 6
...
def AutoconfigurePlatformSpec(env):
    ...
    options = {}
    hostname_f = hostname_f.decode('utf-8')
    march = march.decode('utf-8')
    uname = uname.decode('utf-8')
    uname_all = uname_all.decode('utf-8')
    if hostname_f.find('jscc') >= 0:
        assert march == 'x86_64'
        return PlatformSpecMVS1P5(env, options)
    elif hostname_f.find('ovm-server') >= 0:
        assert march == 'x86_64'
        return PlatformSpecMVS1P5(env, options)
